...
def lattice_point(x,y, x0,y0):
    ...
    return S["V0"]*np.exp( -1/(4* S["σ"]**2) * ((x-x0)**2+(y-y0)**2))
...
def T():
    ...
    (y - S["initial wavefunction offset y"] )**2)) / np.sqrt(2*np.pi* S["σiw"]**2)  *np.exp(p_x0*x*1j)
...
wavefunction_saturation = 0.1, animation_duration = S["animation duration"], 
