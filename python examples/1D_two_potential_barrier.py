...
for i in bar(range(S["store steps"])):
    ...
    if(S["round"]):
       Ψ[i+1] = np.round(tmp, S["round digits"])
    else:
       Ψ[i+1] = tmp 
...
def animate(xlim=None, figsize=(16/9 *5.804 * 0.9, 5.804), animation_duration = 5, fps = 20, save_animation = False, 
    title = "1D potential barrier"):
        ...
        plt.text(S["x0"] - 2.5, 0.95, '+', horizontalalignment='center', verticalalignment = 'center', color = 'red')
        plt.gcf().axes[0].axvspan(S["x1"] - 0.5, S["x1"] + 0.5, alpha=0.5, color='gray')
        plt.text(S["x1"] + 2.5, 0.95, '-', horizontalalignment='center', verticalalignment = 'center', color = 'gray')
...
animate(animation_duration = S["animation duration"], fps = S["fps"], save_animation = S["save animation"], title = "1D two potential barrier")
