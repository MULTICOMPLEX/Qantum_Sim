...
S = {
 ...
 "fps": 30,
 "path save": "./gifs/",
 "title": "2D cyclotron orbit magneticfield momentum space"
}
...
phi_pxpy = np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(Ψ))) * dx**2 / (2 * np.pi)
phi_pxpy = np.abs(phi_pxpy)**2
Ψmax = np.amax(np.abs(phi_pxpy))
Ψ_plot = phi_pxpy/Ψmax
